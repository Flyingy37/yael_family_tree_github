...
export function TimelineView({ persons, filteredIds, onSelectPerson }: Props) {
  ...
  // Auto-scroll to a reasonable starting point
  useEffect(() => {
    if (timelineRef.current && decades.length > 0) {
      const idx1900 = decades.findIndex(d => d >= 1900);
      if (idx1900 > 0) {
        const scrollTarget = (idx1900 / decades.length) * timelineRef.current.scrollWidth;
        timelineRef.current.scrollLeft = scrollTarget - 200;
      }
    }
  }, [decades]);
  ...
  return (
    <div className="w-full h-full flex flex-col bg-white" dir="rtl">
      ...
      <div className="px-4 py-3 border-b border-gray-200 flex items-center gap-3">
        <h2 className="font-bold text-gray-700">ציר זמן</h2>
        <span className="text-sm text-gray-400">{events.length.toLocaleString()} אירועים</span>
        {selectedDecade !== null && (
          <button
      ...
      <div className="flex-1 overflow-y-auto px-4 py-2">
        {selectedDecade === null ? (
          <div className="text-center text-gray-400 text-sm mt-8">
            לחצ/י על עשור בגרף למעלה כדי לראות אירועים
          </div>
        ) : (
          <div>
            <h3 className="font-bold text-sm text-gray-700 mb-2">
              שנות ה-{selectedDecade} — {selectedEvents.length} אירועים
            </h3>
      ...
      <div className="px-4 py-2 border-t border-gray-100 flex gap-4 text-xs text-gray-500">
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded-full bg-blue-400" /> לידה
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded-full bg-red-400" /> פטירה
        </span>
      </div>
    </div>
  );
