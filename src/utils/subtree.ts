...
export function getSubtreeIds(
  ...
  includeSpouseBranches: boolean = true
): Set<string> {
  ...
  function collectAncestors(id: string, depth: number) {
    if (depth < 0 || ancestorVisited.has(id)) return;
  ...
  function collectDescendants(id: string, depth: number) {
    if (depth < 0 || descendantVisited.has(id)) return;
    ...
    for (const famId of person.familiesAsSpouse) {
      ...
      for (const spouseId of family.spouses) {
        ids.add(spouseId);
        if (includeSpouseBranches) {
          // Include spouse branch (parents/ancestors) so spouses are not isolated in subtree view.
          collectAncestors(spouseId, depth - 1);
          // Include spouse descendants (e.g. prior/other branches) within current depth budget.
          collectDescendants(spouseId, depth - 1);
        }
      }
      // Add children
