...
export function MapView({ persons, filteredIds, onSelectPerson, language = 'en' }: Props) {
  ...
  return (
    <div className="w-full h-full relative">
      <div className="absolute top-3 right-3 z-[1000] bg-white/90 backdrop-blur rounded-lg px-3 py-2 text-sm shadow" dir={t ? 'rtl' : 'ltr'}>
        <span className="font-medium">{totalOnMap.toLocaleString()}</span>
        <span className="text-gray-500">{t ? ' אנשים על המפה' : ' people on map'}</span>
        <span className="text-gray-400"> ({points.length} {t ? 'מיקומים' : 'locations'})</span>
      </div>
      <MapContainer
        center={[32.0, 34.8]}
        zoom={3}
      ...
      >
        ...
        {points.map(cluster => (
          ...
          >
            <Popup>
              <div className="max-h-48 overflow-y-auto text-xs" dir={t ? 'rtl' : 'ltr'}>
                <div className="font-bold mb-1 text-sm">
                  {cluster.persons[0]?.birthPlace || `${cluster.lat}, ${cluster.lng}`}
                </div>
                <div className="text-gray-500 mb-2">{cluster.persons.length} {t ? 'אנשים' : 'people'}</div>
                ...
                {cluster.persons.length > 20 && (
                  <div className="text-gray-400 mt-1">+{cluster.persons.length - 20} {t ? 'נוספים...' : 'more...'}</div>
                )}
              </div>
            </Popup>
          </CircleMarker>
